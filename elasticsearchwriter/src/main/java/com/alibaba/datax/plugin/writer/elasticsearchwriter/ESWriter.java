...
public class ESWriter extends Writer {
    ...
    private static Map<String, Object> TABLE_GeoMap = new HashMap<String, Object>();
                ...
                for (Object col : column) {
                    ...
                    switch (colType) {
                        ...
                        case TEXT:
                            ...
                            keyword.put("keyword", tMap);
                            tMap.put("type", "keyword");
                            tMap.put("ignore_above", "256");
                            field.put("fields", keyword);
    ...
    public static class Task extends Writer.Task {
        ...
        private String getDateStr(ESColumn esColumn, Column column) {
            ...
            if (column.getType() != Column.Type.DATE && esColumn.getFormat() != null) {
                ...
                log.info("这个是时间1  " + date.toString());
                return date.toString();
            } else if (column.getType() == Column.Type.DATE) {
                //date = new DateTime(column.asLong(), dtz);
                //log.info("这个是时间2  "+column +column.asLong() +" "+date.toString());
                // log.info("更改后的时间 ："+dateFormat(column.asLong(),FORMAT_FULL_TIME_NO_ZONE));
                //log.info("更改后的时间 ：" + dateFormat(column.asLong(), FORMAT_FULL_TIME_NO_ZONE));
                return dateFormat(column.asLong(), FORMAT_FULL_TIME_NO_ZONE);
            } else {
                log.info("这个是时间3  " + column.asString());
                    ...
                    if (columnList.get(i).isArray() != null && columnList.get(i).isArray()) {
                        ...
                        switch (columnType) {
                            ...
                            case IP:
                                ...
                                break;
                            case GEO_POINT:
                                // log.info("geo coming here!!!!");
                                Object geoPoint = getGeoCache(column.getRawData().toString());
                                if (geoPoint == null) {
                                    log.info("geoPoint is null");
                                    break;
                                }
                                data.put(columnName, geoPoint);
                                break;
                            case BOOLEAN:
        ...
        public Object getGeocenter(String district) {
            String[] geoStrings = {};
            String userkey = "563f8f69d9c4edd109f19abb10cd1ed5";
            String urlBuilder = "https://restapi.amap.com/v3/config/district?" + ("keywords=" + district.trim()) + ("&key=" + userkey.trim());
            String apiresult = HttpClient.sendGet(urlBuilder);
            JSONObject geojson = JSONObject.parseObject(apiresult);
            String status = geojson.getString("status");
            String center = "";
            if ("1".equals(status)) { // 获取成功
                log.info("获取经纬度成功:" + district);
                JSONArray districtjson = geojson.getJSONArray("districts");
                if (districtjson.isEmpty()) {
                    log.info(district + "的经纬度为空");
                    return null;
                }
                center = districtjson.getJSONObject(0).getString("center");
                log.info("center: " + center);
                geoStrings = center.split(",");
            } else {
                log.info("获取经纬度失败:" + geojson.getString("info"));
                return null;
            }
            if (geoStrings.length < 2) {
                log.info("获取经纬度失败:数值为空");
                return null;
            }
            GeoPoint geoPoint = new GeoPoint(Double.parseDouble(geoStrings[1]), Double.parseDouble(geoStrings[0]));
            return geoPoint;
        }

        public void addGeoCache(String district, Object geoPoint) {
            if (!TABLE_GeoMap.containsKey(district)) {
                TABLE_GeoMap.put(district, geoPoint);
            }
        }

        public Object getGeoCache(String district) {
            if (TABLE_GeoMap.containsKey(district)) {
                return TABLE_GeoMap.get(district);
            }
            Object geoPoint = getGeocenter(district);
            addGeoCache(district,geoPoint);
            return geoPoint;
        }

        public void updateGeoCache(String district) {
            if (!TABLE_GeoMap.containsKey(district)) {
                Object geoPointOne = TABLE_GeoMap.get(district);
                if (geoPointOne == null) {
                    Object geoPoint = getGeocenter(district);
                }
            }
